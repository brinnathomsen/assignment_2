...
function draw() {
  ...
  if (boatLocation < 640) {
    currentScene = random(0, 10);
    currentSkyWater = random(0, 100);


  } else {

    currentScene = currentScene;
    currentSkyWater = currentSkyWater;
  }



  skyWater(currentSkyWater);
  scene(currentScene);
  ...
  if (weather > 70) {
    ...
    rect(boatLocation - 30, 270, 65, 10);
  } else if (weather > 50) {
  ...
  if (weather > 80) {
    ...
    if (i = 0, i < 100, i++) {
      var snowflakeSize;
      fill(255, 255, 255);
      snowflakeSize = random(4, 8) * i;
      ellipse(random(0, 640) * i, random(0, 360) * i, snowflakeSize,
        snowflakeSize);
    }
